...
Diese Repository dient als "prove-of-concept" für eine Integration zwischen einer Authentifizierungssoftware und der Webex Umgebung. Ziel des Projektes ist es, Unternehmen mehr Kontrolle und Sicherheit zu gewährleisten, indem man (vor allem Unternehmensexterne) Meeting-Teilnehmer über sichere dritte Stelle authentifiziert bevor diese ein Meeting beitreten. Auf diesen Grund kann "Inpersonation" weitestgehend verhindert werden, was es sicherer macht, sensible Informationen weiterzugeben. Auch die Meetings an sich werden sicherer, weil Meeting Informationen nicht direkt mit User geteilt werden. Sollten diese dennoch freigegeben werden, sind nicht-authentifizierte Benutzer trotzdem nicht in der Lage ein gesichertes Meeting beizutreten.

## Dokumentation

Folgend werden die technischen Aspekte der Integration im Einzelnen beschrieben, erklärt und begründet. Das System besteht aus mehreren, voneinander unabhängigen Bauteile, nämlich:
...
Das Projekt wird immer aus der Sicht zweier Aktoren beschrieben, der Meeting-erstellen und der Meetingteilnehmer.
...
Dieser JWT muss dann in einen OAuth2 Token umgewandelt werden (ist noch eine Limitierung von Webex). Mit diesen Token kann man nun Meetings beitreten.
...
Die Webapp kann auf allen gängigen Betriebssysteme ausgeführt werden. Ggf. werden Admin Rechte benötigt für die Installation der nötigen Software.
...
Ein Neustart ist nicht immer notwendig, je nachdem, was man alles bereits installiert gehabt hat. Falls alles frisch unter Windows installiert wurde, wird ein Neustart empfohlen.
...
Ansonsten unter Code auf ZIP herunterladen klicken und irgendwo entpacken.
...
1. `login` - Öffnet ein Webex Login Fenster. Hier kann man die Webex Integration autorisieren, damit man einen Zugang zu der Webex API bekommt. Falls das erfolgreich gewesen ist, wird auf der Seite einen Code angezeigt.
